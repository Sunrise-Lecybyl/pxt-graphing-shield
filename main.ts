    ...
    export function pieChart(buckets: Array<number>) : void {
        ...
        for (let bucket of normalised) {
            ...
            for (let i = dir; i <= new_dir; i+=1) {
                ...
                screen().drawLine(80 + 25 * Math.sin(dir * Math.PI / 180), 60 + 25 * Math.cos(dir * Math.PI / 180), 80 + 50 * Math.sin(angle), 60 + 50 * Math.cos(angle), colour);
                screen().drawLine(80 + 25 * Math.sin(new_dir * Math.PI / 180), 60 + 25 * Math.cos(new_dir * Math.PI / 180), 80 + 50 * Math.sin(angle), 60 + 50 * Math.cos(angle), colour);
            }
            colour++;
            dir = new_dir;
        }
